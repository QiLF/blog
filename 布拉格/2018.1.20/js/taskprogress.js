...
/*
函数说明：添加选项卡的标题
参数：任务编号             暂时认为任务后端传来的task_name是下标从0开始
返回：无
*/
function add_tab_title(task_label)
{
	if(task_label===1)
	{
		$("#tab_title").append("<li class='layui-this'>"+task_name[task_label-1]+"</li>");
	}
	else
	{
		$("#tab_title").append("<li>"+task_name[task_label-1]+"</li>");
	}
}
...
function add_tab_item_content(task_label)
{
	...
	$("#task"+task_label).append(
								...
								+			"参与成员："+task_collaborator[task_label-1]
								+		"</div>"								
								+ 	"</div>"
								+"</div>"
								);
								
	add_progress_bar(task_label,"50%");
	...
	add_total_sub_task(task_label,sub_task_name[task_label-1].length);
}
/*
...
//,sub_task_situation)
{
	...
	var percent=0;
    if(Array.isArray(sub_task_situation[task_label-1]))
	{
       for(var i=0;i<sub_task_situation[task_label-1].length;i++)
	   {
	      change_one_face('face'+task_label+'-'+(i+1),sub_task_situation[task_label-1][i]);
		  if(sub_task_situation[task_label-1][i]==true)sub_finish_num++;
	   }
	   percent=sub_finish_num/sub_task_situation[task_label-1].length;
    }
