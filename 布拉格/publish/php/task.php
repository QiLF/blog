<?php
    ...
    if(empty($_POST["res"])){
        ...
        if(!$state){
            ...
            $sql = "INSERT INTO `class`.`tasks` (`task_id`, `name`, `introduction`, `start_date`, `end_date`, `priority`, `evaluation`, `builder`) VALUES ('$task_id', '{$data["name"]}', '{$data["introduction"]}', '{$data["start_date"]}', '{$data["end_date"]}', '{$data["priority"]}', '-1', '{$data["builder"]}');";
            if(!mysqli_query($con, $sql)){
                ...
                foreach($data["subtasks"] as $subtask){
                    ...
                    $sql = "INSERT INTO `class`.`subtasks` (`subtask_id`, `name`, `start_date`, `end_date`, `progress`, `evaluation`) VALUES ('$subtask_id', '{$subtask["name"]}', '{$subtask["start_date"]}', '{$subtask["end_date"]}', '0', '-1');";
                    if(!mysqli_query($con, $sql)){
                        echo json_encode(array("success"=>"false", "error"=>mysqli_error($con), "res"=>""));
                        mysqli_close($con);
                        return;
