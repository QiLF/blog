 ...
 //一项任务的预览
  function task_view(task_i)
  {
	...
	if(tasks[task_i].evaluation==-1){
		task_evaluation='<span class="layui-text">暂无</span>';
	}else{
		task_evaluation=tasks[task_i].evaluation;
	}
	document.getElementById("task_show_evaluation").innerHTML='<span class="layui-text" style="font-size:20px;color:#FFB800">'+task_evaluation+'</span>';
	...
	for(var i=0;i<subtasks_num;i++)
	{
		...
		if(tasks[task_i].subtasks[i].evaluation==-1){
			subtask_evaluation='<span class="layui-text">暂无</span>';
		}else{
			subtask_evaluation='<span class="layui-text" style="font-size:20px;color:#FFB800">'+tasks[task_i].subtasks[i].evaluation+'</span>';
		}
